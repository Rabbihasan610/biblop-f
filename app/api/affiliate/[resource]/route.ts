...
const routes: Record<string, { path: string; methods: string[]; public?: boolean }> = {
  ...
  deposit: { path: '/api/affiliate/deposit', methods: ['POST'] }, withdraw: { path: '/api/affiliate/withdraw', methods: ['POST'] }, logout: { path: '/api/affiliate/logout', methods: ['POST'] },
};
...
async function proxy(request: NextRequest, resource: string) {
  ...
  const store = await cookies(), token = store.get('jaba9_affiliate_token')?.value;
  ...
  try {
    ...
    if (resource === 'login' && response.ok && typeof accessToken === 'string') outgoing.cookies.set('jaba9_affiliate_token', accessToken, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/', maxAge: 60 * 60 * 24 * 7 });
    if (resource === 'logout') outgoing.cookies.delete('jaba9_affiliate_token');
    return outgoing;
