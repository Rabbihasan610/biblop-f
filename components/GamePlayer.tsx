...
export function GamePlayer({ slug, id }: { slug: string; id?: string }) {
  ...
  useEffect(() => {
    fetch(`/api/public/games?search=${encodeURIComponent(slug)}&per_page=20`, { headers: { Accept: 'application/json' } }).then(response => response.json()).then(payload => {
      const raw = Array.isArray(payload?.data) ? payload.data : payload?.data?.data || payload?.data?.games || [];
      const found = raw.find((item: Record<string, unknown>) => String(item.slug) === slug || String(item.id) === id) || raw[0];
      if (!found) return;
      setGame({ ...fallback, ...found, id: Number(found.id || fallback.id), name: String(found.name || fallback.name), slug: String(found.slug || slug), launch_url: String(found.launch_url || '') }); setFallbackMode(false);
    }).catch(() => setFallbackMode(true));
  }, [fallback, id, slug]);
  async function startGame() {
    try {
      const response = await fetch('/api/public/click', { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ game_id: game.id }) });
      const payload = await response.json();
      if (response.ok && payload.status === 'success' && payload.data?.launch_url) setGame(current => ({ ...current, launch_url: String(payload.data.launch_url) }));
