...
import { fallbackUser } from '@/lib/fallback-data';
import { Icon, type IconName } from './Icons';

type Json = Record<string, unknown>;
type Section = 'dashboard'|'deposit'|'withdraw'|'transactions'|'profile'|'password'|'logs';
const nav: [Section, string, IconName][] = [['dashboard','Dashboard','home'],['deposit','Deposit','wallet'],['withdraw','Withdraw','arrow'],['transactions','History','history'],['logs','Game logs','games'],['profile','Profile','user'],['password','Password','user']];
...
export function UserArea({ section }: { section: Section }) {
  const [data, setData] = useState<Json>(fallbackUser), [records, setRecords] = useState<Json[]>([]), [source, setSource] = useState<'api'|'fallback'>('fallback');
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (section === 'dashboard') { const dashboard = await request('dashboard'); setData(dashboard); const result = await request('transactions').catch(() => ({} as Json)); const value = (result.transactions || result.data || result) as Json | Json[]; setRecords(Array.isArray(value) ? value : []); }
      if (section === 'profile') setData(await request('profile'));
      if (section === 'transactions') { const result = await request('transactions'); const value = (result.transactions || result.data || result) as Json | Json[]; setRecords(Array.isArray(value) ? value : []); }
      if (section === 'logs') { const result = await request('game-logs'); const value = (result.logs || result.data || result) as Json | Json[]; setRecords(Array.isArray(value) ? value : []); }
      ...
      setSource('api');
    } catch { setSource('fallback'); if (section === 'dashboard' || section === 'profile') setData(fallbackUser); setRecords([]); }
  ...
  return <main className="account-layout page-enter">
    ...
    <section className="account-content"><header className="account-heading"><div><p className="eyebrow">{section === 'transactions' ? 'Account activity' : 'Wallet & account'}</p><h1>{nav.find(([key]) => key === section)?.[1]}</h1></div><span className={`source-badge ${source}`}>{busy ? 'Loading…' : source === 'api' ? 'Live API data' : 'Fallback data'}</span></header>
      ...
      {section === 'logs' && <section className="account-card"><div className="card-title"><h2>Game logs</h2></div><TransactionRows rows={records}/></section>}
      {section === 'password' && <form className="account-form account-card" onSubmit={event => void submit(event, 'password')}><div className="form-grid"><label>Current password<input name="current_password" type="password" required/></label><label>New password<input name="password" type="password" minLength={8} required/></label><label>Confirm password<input name="password_confirmation" type="password" minLength={8} required/></label></div><button className="button-yellow" disabled={busy}>Change password</button></form>}
